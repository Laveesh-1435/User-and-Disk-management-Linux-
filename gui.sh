    ...
    while true; do
        ...
        case "$choice" in
            "1")
                ...
                ;;
            "2")
                alert_threshold=$(whiptail --inputbox "Enter disk usage threshold (%) for alert:" 10 60 "80" --title "Check Disk Usage" 3>&1 1>&2 2>&3)
                recipient_email=$(whiptail --inputbox "Enter recipient email for alerts (optional):" 10 60 "" --title "Check Disk Usage" 3>&1 1>&2 2>&3)
                check_disk_usage "$alert_threshold" "$recipient_email" | less
                ;;
            "3")
