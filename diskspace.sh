...
check_disk_usage() {
    ...
    if [[ "$usage" -gt "$1" ]]; then
        ...
        if [[ -n "$2" ]]; then
            alert_message+="Sending alert email to $2...\n"
            echo "Subject: Disk Space Alert on $HOSTNAME" | mail -s "Disk Space Alert on $HOSTNAME" "$2" <<< "Disk space on $mount_point is critically low: $usage%"
        fi
    else
